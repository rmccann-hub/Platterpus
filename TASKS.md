...
### ⭐ Current plan & priorities (set 2026-06-09 after a full doc audit — START HERE)

The live, ordered work queue. Difficulty: **S** = a focused session, **M** = 1–2 sessions (may need upstream-source research), **L** = multi-session. **HW** = hardware-gated (needs the user's machine/drive/disc — code-side prep can still be done first). The "Ranked execution order" list further down is the *historical* record of how we got here; this section is what to pick up next.

1. **[ ] cyanrip stdout progress parsing — M.** During a cyanrip rip the progress bars sit still and the status line goes stale (completion, success detection, and `.log` discovery already work). Research cyanrip's actual rip output (likely `\r`-terminated progress lines — may need the line-streaming in `RipHandle.log_lines()` to split on `\r` too), then a `_progress_for`-equivalent for cyanrip in `RipWorker`. *Why first: it's the last piece that makes a cyanrip rip feel like a working program, and it must land before the hardware parity run is meaningful.*
2. **[ ] cyanrip log fidelity verdict — M.** Parse cyanrip's `.log` (EAC CRC32 per track, AccurateRip v1/v2 results) into the existing `RipLog`/track-results shape so the "Done — all N tracks verified" summary and the AccurateRip table work on both backends. New `parsers/cyanrip_log.py` + never-raises property test (institutional rule).
3. **[ ] Hardware parity run — HW (user).** Real wizard run installing cyanrip (COPR step), then the Police disc through both backends: compare CRCs, confirm cyanrip clears the track-3 failure (+667 >587 bug). Findings feed fixes; this closes KDD-18 or reopens it with facts.
4. **[ ] In-app Uninstaller — M/L (standing user request, 2026-06-08).** GUI flow (Tools → Uninstall…) that removes *our* stuff — menu/desktop shortcuts, GUI config + logs, host-exported `whipper`/`metaflac`/`cyanrip`, the `ripping` container, optionally the AppImage file itself — and **keeps distrobox/podman and all music**. Build it as the reverse arm of `deps/host_setup.py` (same injectable-runner pattern, dry-run, per-step report). Include the `uninstall.sh` parity gaps (remove `~/.local/bin/cyanrip`) — S — in the same change.
5. **[ ] App + Uninstaller desktop icons & Multimedia menu category — S (same user request).** Verify/fix the `.desktop` categories so the app lands under Multimedia, add an uninstaller launcher icon; extend `appimage_integration.py`.
6. **[ ] AppImage self-update (zsync) — M (KDD-17b, the last zero-CLI slice).** Embed update-information in the AppImage build, verify against the published `.sha256`; use the standard AppImageUpdate flow, no hand-rolled downloader.
7. **[ ] setup-host.sh `--cyanrip` parity — S.** The CLI equivalent of the wizard's cyanrip step (COPR repo file + dnf install + export), same stanza, same idempotency.
8. **[ ] CTDB verify validation — HW (user) → then GUI wiring — M.** Run [docs/test-plan.md](docs/test-plan.md) Test 1 with a pressed disc that's in CTDB; fix `toc=`/CRC per the documented seams; then wire verify into the GUI (also export host `flac` in the wizard for the decoder). CTDB **repair** (L, .NET bundling question) stays parked until verify is proven.
9. **[ ] Remaining hardware-gated doc items — HW (user), S each.** Real-run confirmations: `setup-host.sh` / host wizard from scratch, `drive analyze`/`offset find` success strings (test-plan Tests 3–4), README screenshots (Test 5), Picard UX (Test 6), PyPI Trusted-Publisher one-time setup (Test 7).

*Parked / later (unchanged ranking):* MP3+WAV encoders via the dep subsystem (M), multi-disc queue, multi-drive, udev auto-detect, ReplayGain, library auto-move (all P1 backlog); EAC-style signed log checksum (LOW); upstream whipper bug-fix PRs (opportunistic).
...
- **[~] ⭐ Zero-CLI distribution (PLANNING.md [KDD-17](PLANNING.md); user-approved 2026-06-04; ranked-order item 7).** The headline install-ease goal: download one file, double-click, no terminal ever. Ship in three independent slices:
  - **[x] (a) Self-integrating AppImage — SHIPPED 2026-06-05** (`appimage_integration.py`; details in ranked-order item 7).
  - **[ ] (b) Self-updating AppImage.** Embed AppImage **update-information (zsync)** so the app checks for a newer release, downloads only the delta, and skips if already current — verified against the published `.sha256`. This is the user's "stub reads a manifest and fetches the right version" idea, done the standard way; **do not hand-roll a download stub** (extra code + supply-chain surface, reinvents AppImageUpdate). *The remaining slice.*
  - **[x] (c) GUI first-run host wizard — SHIPPED 2026-06-05** (`deps/host_setup.py` + dialog + worker; cyanrip step added 2026-06-09; details in ranked-order item 7). Note: the "also export `flac` from the container" idea for CTDB verify is **not** done — fold it into the wizard when CTDB verify gets GUI-wired.
