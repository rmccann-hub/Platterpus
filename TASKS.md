...
- **[x] Default path template → Artist/Album folders, "## - Title" files.** Done 2026-05-29 (T32). The v1 default (`%A - %d/%t. %a - %n`) produced a flat `Artist - Album/` folder; the new default is `%A/%d/%t - %n` (track) + `%A/%d/%d` (disc), giving `Artist/Album/01 - Title.flac`. Config schema bumped to v2 with a migration that upgrades the templates only if they still hold the v1 defaults (a hand-edited template is preserved). Still fully editable in Settings.

- **Friendly folder name for unknown discs.** For a disc MusicBrainz can't identify, whipper substitutes the raw disc-ID hash for `%d`, so the album folder becomes e.g. `Unknown Artist/PKt4tUZ9zkm.../`. Deferred during T32 (the hash is unique and collision-free). Implement a post-rip rename (and file re-template) to something like `Unknown Album` or `Unknown Album - <date>`. Touches the rip-finish path; needs care around the already-applied placeholder tags and the `.log`/`.cue` paths that embed the old folder name.

- **Highlight the current track row during a rip.** The track table doesn't follow along as whipper rips track N — the selection stays wherever the user last clicked. Low priority (cosmetic), surfaced in T32. The `RipWorker.progress(track, percent)` signal already carries the current track number, so the main window could select/scroll that row in the track table on each progress tick.
...
- **Edited track tags feed the unknown-album rip.** The track table now shows numbered blank rows for a disc MusicBrainz can't identify (see P1 UX gaps), and the rows are editable. But an unknown-mode rip ignores those edits: whipper writes placeholder `Track NN` / Unknown Artist / Unknown Album tags and the post-rip step applies them. Wiring the user's edits through to the FLAC tags (instead of, or layered over, the placeholders) is the natural next step. Bounded: read `TrackTable.album_metadata()` + `tracks()` after an unknown rip and pass them to `apply_placeholder_tags` (which would become `apply_tags`).
