...
9. **[ ] `CyanripImpl` successor backend (PLANNING.md [KDD-18](PLANNING.md), decided 2026-06-04).** Strategic, medium-term: whipper is stalled (last release 2021-05) with a `pkg_resources`/Python-3.14 cliff; **cyanrip** (active, C/FFmpeg, LGPL-2.1, AccurateRip v1/v2 + EAC CRC) is the successor. Build it behind the existing `WhipperBackend` ABC as a config-selectable second backend; **never fork whipper**. Phased plan (feasibility spike → adapter → parser → Settings selector → parity tests → hardware parity run) + open packaging unknown in [docs/ecosystem-audit-2026-06.md](docs/ecosystem-audit-2026-06.md). CTDB is backend-independent and unaffected.
...
- **[ ] ⭐ Zero-CLI distribution (PLANNING.md [KDD-17](PLANNING.md); user-approved 2026-06-04; ranked-order item 7).** The headline install-ease goal: download one file, double-click, no terminal ever. Ship in three independent slices:
  - **[ ] (a) Self-integrating AppImage.** On first run, offer "Add to application menu?" and install the app's own `.desktop` + icon from inside the AppImage; supersedes the *user-facing* `install-appimage.sh` (which stays for scripted/CI use). Smallest slice; ship first.
  - **[ ] (b) Self-updating AppImage.** Embed AppImage **update-information (zsync)** so the app checks for a newer release, downloads only the delta, and skips if already current — verified against the published `.sha256`. This is the user's "stub reads a manifest and fetches the right version" idea, done the standard way; **do not hand-roll a download stub** (extra code + supply-chain surface, reinvents AppImageUpdate).
  - **[ ] (c) GUI first-run host wizard.** The larger lift and the bigger win: move `setup-host.sh`'s work — create the `ripping` container, install whipper/flac/metaflac in it, `distrobox-export` — into a buttons-and-progress wizard **owned by the dependency self-management subsystem** (Critical Rule #6), driving **rootless** podman/distrobox directly. On Bazzite/Silverblue the runtime is preinstalled → fully click-driven, no elevation. On distros lacking the runtime, **one polkit-elevated step** installs it (the only unavoidable prompt; atomic distros may also need a reboot). `setup-host.sh` remains the CLI equivalent. Also export `flac` from the container here (CTDB verify needs host `flac`).
