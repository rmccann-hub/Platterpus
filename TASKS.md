...
5. **CTDB verify (read-only)** — Phase 1 of KDD-14; foundation for repair. **← next up; license gate now closed.** Build clean-room from the LGPL reference (KDD-16, 2026-06-02 — see [docs/upstream-modification-investigation.md](docs/upstream-modification-investigation.md) for the LGPL-grounded protocol/CRC spec). Pure-Python client (not a `ctdb-cli` wrap). Only remaining blocker: hardware validation against a real CD that's in CTDB.
