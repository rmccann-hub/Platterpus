...
### P1 — Rig-session findings, 2026-08-05 (b10 + cyanrip `9048082`) — RECORDED, NOT STARTED

Raised by the maintainer while walking through the b10 build on the Bazzite rig. **He asked for these to be recorded and explicitly said not to act yet** — he is still uploading data and will give a go-ahead. Every claim below was verified against the code *before* being written down, because two of the five turned out not to be what the screenshots suggested.

- **[ ] Goal preset labels should name the whole configuration, e.g. `FLAC — Lossless Archival Master [Debugging]`.** Maintainer's words: *"These settings should be called something like Flack - Lossless Archival Master [Debugging] or similar, and other settings should reflect similar naming syntax."*
...
- **[ ] My own instruction was wrong: `./platterpus-x86_64.AppImage --install-ripper 9048082` fails after the app relocates itself.** The AppImage is **moved** (`shutil.move`, `appimage_integration.py:93`) to `~/Applications/` — but only after an explicit Yes, and the app *does* name the new absolute path in a follow-up dialog (`main_window_provision.py:192-199`). So the app behaved correctly and the docs did not: sweep every bare `./platterpus-x86_64.AppImage …` command in `README.md` and `docs/` and make them relocation-safe. Also worth noting for the record: the b10 wizard built `9048082` on its own, so the manual step was never needed.
