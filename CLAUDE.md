...
- **Critical Rule #3 — one approved exception (2026-05-31): force-stopping a cancelled rip.** A rip's `cdparanoia` runs inside the `ripping` container; podman doesn't forward the host SIGKILL, so the drive can spin for minutes after Cancel (the physical eject button is inhibited while the reader holds the device, so pressing eject by hand does nothing either). `drive_control.force_stop_drive()` stops it: **(1) host `pkill -KILL` of the reader by process *name*** (`cdparanoia|cd-paranoia|cdrdao`) — on rootless podman/Distrobox the in-container reader is a host-visible process, so this reaches it; **(2) fallback `distrobox enter ripping -- pkill -KILL <names>`** only if the host pkill matched nothing (the user-approved into-the-container call); **(3) then `eject`** (only works once the reader has released the device). **Two hard-won gotchas:** never use `pkill -f` (full-cmdline) — it matches the GUI's own "whipper-gui" cmdline and the `distrobox enter … whipper …` wrapper, killing the app/session instead of the reader (real-user bug, 2026-06-01); and never include "whipper" in the pattern for the same reason. Match reader names only. `pkill`/`distrobox`/`eject` resolved to absolute paths (desktop-launched GUI has minimal PATH). **Scope is strictly force-stop on cancel** — ripping still goes through `~/.local/bin/whipper`. MainWindow auto-escalates `_FORCE_STOP_COUNTDOWN_MS` (5s) after Cancel, plus a manual "Force stop" button. Do NOT widen this exception without asking.
