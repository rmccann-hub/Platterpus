...
12. **The cyanrip seam is bidirectional, and both directions are enforced by tooling — not by memory.** Platterpus and the cyanrip fork are each other's dependency: we consume its log and argv surface, its log format exists to satisfy us. **Permanent, standing behaviour on both sides** (`docs/cyanrip-handshake.md` §7):
    ...
    - **An upstream change cannot be escaped by rolling back to upstream.** The `-V` removal came from upstream, not the fork, so "revert to stock cyanrip" was never a mitigation — only pinning 0.9.3 or fixing the probe was. When planning a rollback, check whether the failure is *ours*, the fork's, or upstream's; the third kind has the fewest exits.
