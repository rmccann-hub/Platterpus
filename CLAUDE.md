...
- **Would this test fail if I reverted the fix?** Check by actually reverting it. This has caught a vacuous detector here **twice** — including one whose first version passed against the very bug it was written for, because it looked for a *mention* of a thread rather than a *call* that stops it. **And verify the revert actually landed before believing the run**: a `str.replace`-style revert silently no-ops when the formatter has reflowed the line you are matching, and a revert that did not apply produces a *passing* test that looks exactly like a vacuous one (2026-08-03 — third silent no-op of this shape in the project). Re-read the file, or revert with an editor that fails loudly.
...
- **Am I verifying the behaviour, or the other side's *description* of the behaviour?** Different claims, and the second is easy to mistake for the first. Round 4 of the cyanrip handshake reported their 88-string fatal inventory as "VERIFIED INDEPENDENTLY" — what was verified was *the 88 strings they sent*, run through our real pattern. Re-deriving the inventory from their source in round 5 found two more, missed by a systematic blind spot in their generator. A list checked against itself is consistent, not verified. **Where the underlying source is reachable, derive from it.**
