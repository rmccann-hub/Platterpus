...
# The freedesktop app-id drives the main .desktop filename and the bundled icon
# name (io.github.rmccann_hub.Platterpus), matching the AppStream metainfo
# <id> and the recipe's Icon= field. The secondary "uninstall" launcher uses
# the plain APP_NAME slug (it isn't an app in its own right).
DESKTOP_ID = APP_ID
_DISPLAY_NAME = "Platterpus"
...
CANONICAL_APPIMAGE_NAME = "platterpus-x86_64.AppImage"
...
def integrate(
    ...
    *,
    ...
    # A separate "Uninstall Platterpus" menu entry (menu only — deliberately
    ...
    uninstall_entry = desktop_dir / f"{APP_NAME}-uninstall.desktop"
