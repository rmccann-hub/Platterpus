...
#: A line a logchecker could read as a log SIGNATURE: EAC's
#: `==== Log checksum <hex> ====`, or one of our own two footer lines. Matched
#: anywhere in a line and in any case, because a checker's pattern need not be
#: anchored. Our body never writes one: this can only come from metadata.
_SIGNATURE_SHAPE: re.Pattern[str] = re.compile(
    r"={2,}\s*(?:log checksum|platterpus log checksum|this log was generated by platterpus)",
    re.IGNORECASE,
)
...
def _defuse_signature_lines(lines: list[str]) -> tuple[list[str], list[str]]:
    ...
    The log echoes the album line and file names as EAC does, so an album artist
    written as `==== Log checksum <64 hex> ====` would put EAC's signature shape in
    our log, and a tracker's logchecker could count the log as EAC-signed. That
    is the forgery KDD-24 forbids, made on our behalf by a metadata value. Our own
    checksum is not at risk (it reads only the last footer line).
...
def render_eac_style_log(
    ...
    *,
    ...
    """The rendered log text; see :func:`render_eac_style_log_and_defused`."""
    text, _defused = render_eac_style_log_and_defused(
        rip_log,
...
def render_eac_style_log_and_defused(
    rip_log: RipLog,
    *,
    platterpus_version: str = "",
    build_fingerprint: str = "",
    ...
    and the lines rewritten so metadata could not forge a signature (D16).
...
def _render(
    ...
    * **labelled** — cyanrip doesn't report it, so the row says
    ...
    lines.append("")
    # Before our own not-signed line goes on, so it is never touched: nothing a
    # metadata value put above it may read as a signature (D16).
    lines, defused = _defuse_signature_lines(lines)
