...
- **Platterpus:** `0.6.60` — the build that
  generated this file. A row can only have changed with our code, so this
  version *is* the range on our half.
- **Verified against ripper build:** `cyanrip 0.9.4-rc2+platterpus.16 (platterpus-fork-g221a1df)` — the build a
  closed handshake round approved (round 27, for Platterpus
  `0.6.60`). Rows in §1 were checked against that build's output;
