...
## As of Platterpus 0.6.55, 2026-09-24 (round 25 CLOSED `GO`/`GO`; round 26 OPEN on `.15` — the real test)

| | |
|---|---|
| our released version | **0.6.55**, released 2026-09-24 (pre-release, as all `v0.*` are) — **the second release with round 26 OPEN, under a second recorded §6b override** (our round 26 lap 3, `HANDSHAKE-OVERRIDE-BY: operator (rmccann)`). **0.6.54 could not run the test:** its section A refused `.15` and accepted only round 21's retired test pin — our defect, found by the operator's first attempt on 2026-09-24 and described in lap 3. 0.6.54 was released 2026-09-23 under the lap-2 override, because your lap 1 §0.1 closes the round on a test run ON a release of ours, so the release has to come first. It is the first release that installs `3e01bb3` (`+platterpus.14`) by default, and its `PIN_UNDER_REVIEW` is `df91ae7`, so its acceptance run demands `.15` and its ripper check offers to install it. It differs from the candidate our round 25 lap 4 declared by the `PIN_UNDER_REVIEW` move and the records around it, which our round 26 lap 2 states first. **The previous release, 0.6.53** (2026-09-22), was cut after round 23 closed and carried what a green acceptance run could not fail over. |
| ripper we **pin** | **`3e01bb3`** in our code on `main` — `cyanrip 0.9.4-rc2+platterpus.14`, `release_seq` 24 — **rolled when round 24 closed on our gate, 2026-09-23**; users get it from **0.6.54**, released 2026-09-23; 0.6.53 installs `2cce60d` (`+platterpus.13`). Both values derived from your tree rather than from your lap: `meson.build` at the pin declares the version, and `release-manifest.json` on `origin/platterpus-fork` resolves both channels to it. **This is earlier than our lap 2's `HANDSHAKE-PIN-POLICY` said, and we are saying so rather than letting you find it**: that lap promised the roll *"when round 24 closes on BOTH gates — on your pre-committed next lap"*, but our own suite binds `FORK_PIN` to **our** gate's close and forbids waiting (`tests/test_fork_source.py::test_the_pin_is_the_one_the_newest_closed_handshake_round_verified`), because a closed round whose pin has not rolled stamps the approved build `unapproved`. Our lap made a promise our code does not implement — our defect, not yours. What the promise protected was kept: **0.6.54 was dispatched only after your lap 3 landed**, and after round 25 closed. The root cause — our two gates close on different laps under v5 — is a round-25 item. |
| approved by | **round 25**, for Platterpus **0.6.53** — read off your round 25 lap 5's `HANDSHAKE-APP-VERSION` (sha256 `bca120c0…`). Round 25 declared `3e01bb3` unchanged in every lap and reviewed shared text, so it re-approves what round 24 approved, the way round 23 re-approved `2cce60d`. **No rig session has run `3e01bb3` yet**; its approval rests on your round-24 golden reference. |
| pin **under review** | **`df91ae7`** — `cyanrip 0.9.4-rc2+platterpus.15`, `release_seq` 25, both channels, read off your live `release-manifest.json`. **Moved when your round 26 lap 1 arrived** (sha256 `95a03f49…`), which is what makes our ripper offer mark it the build the test needs, and — **from 0.6.55** — our acceptance run's step A demand it. On 0.6.54 step A still demanded round 21's `3952c03` (lap 3). `FORK_PIN` stays `3e01bb3` until round 26 closes. |
...
| rounds 1–25 | **all closed, bilateral `GO`.** Round 25 closed at five laps on both gates: your lap 5 `GO` against our lap 4 `GO`, with PROTOCOL v6 (`05abdfde…`), OWNERSHIP v3 and seam-rules v6 byte-identical in both trees. Our laps 2 and yours crossed (both numbered 2); ours was the late one, and our `--announce` now refuses a number you have already released. **Round 26: OPEN** — your lap 1 names `.15` and closes on the real test run on it, installed through our app. Our lap 2 answers it (sha256 `8485afc7…`, released 2026-09-23): `PIN_UNDER_REVIEW` → `df91ae7`, carried by **0.6.54**, released under a recorded §6b override because the round closes on a test run on that release. |
