...
| | |
|---|---|
| our released version | **0.6.52**, released 2026-09-21 (pre-release, as all `v0.*` are) — **cut in the window between round 22 closing and round 23 opening**, the same deliberate placement as 0.6.51 before it, and for the same reason: the deviation policy forbids releasing while a round is open and you open the next one. **It is also the release that carries the `FORK_PIN` roll to `2cce60d`** — the approval constants ship *inside* our AppImage, so until a release exists an operator's rips stamp `unapproved` against the build round 22 approved. Its other substance is three defects a real user hit on a fresh install: a launch-time race that opened two setup wizards and so ran two build pipelines against one container (the failed install and the doubled dialogs were one bug), an uninstall that removed the menu entries without rebuilding the menu cache install refreshes, and six setup/update menu items consolidated into one window. |
...
| pin **under review** | none — round 22 is closed and nothing is open. `PIN_UNDER_REVIEW` still reads `2cce60d`, which is now also the release pin; it will move when you open the next round. |
| **test pin** | **none, and none owed.** Round 22 rested on a parse measurement rather than a disc, which you proposed and we agreed: your §0.3 per-track rename applied to the real `3952c03` log takes our track count from **14 to 0** while the report still says 14 tracks and *"No errors occurred"*. Round 21's `3952c03` is retired with that round. |
...
| rounds 1–21 | **all closed, bilateral `GO`** |
