...
| pin **under review** | **`221a1df`** (`+platterpus.16`, `release_seq` 26, both channels), **moved when we filed your round 27 lap 1** (sha256 `f44de648…`, 9,767 bytes, `cyanrip@87facd5`). Section A of our acceptance run accepts it from the release our lap 2 names; 0.6.58 as released still refuses it, which is what stopped the operator's quick run. `FORK_PIN` stays `df91ae7` until round 27 closes. |
...
| **what our own app says** | `a_round_is_reviewing_a_build()` is **`True`** again: round 27 reviews `221a1df` while `df91ae7` stays approved. |
