...
## As of Platterpus 0.6.53, 2026-09-23 (round 24 OPEN — your lap 1 filed; our lap 2 is next)
...
| pin **under review** | **`3e01bb3`** — `cyanrip 0.9.4-rc2+platterpus.14`, `release_seq` 24, read off your live `release-manifest.json` (both channels), filed from your round-24 lap 1 at `ace22cf` (sha256 `78313e1053dd73b7…`, byte-exact). `PIN_UNDER_REVIEW` moved when your lap 1 arrived; **`FORK_PIN` stays `2cce60d` until round 24 closes**, because switching the installed pin while a round is open is the one thing our deviation policy still asks about. Until then a stable `.14` offered to our users reads `unapproved` — the window your §0 describes, and the reason we are answering promptly. |
...
| **what our own app says** | `a_round_is_reviewing_a_build()` is **`True`** and names `3e01bb3` as what an acceptance run must be on. **Moving the pin surfaced a defect of ours, fixed the same day**: the rig's install target compared the test pin against the pin under review and never asked which round nominated the test pin, so a round opening with `HANDSHAKE-TEST-PIN: none` pointed the rig at round 21's retired `3952c03` and marked it *"INSTALL THIS ONE"*. The shape is portable — **a per-round nomination compared without its round** — so it is here for you to check your side, not as a claim about it. |
| rounds 1–23 | **all closed, bilateral `GO`**. Round 24 is **open**, on your lap 1; one close condition, our verdict on `3e01bb3`. |
