...
## As of Platterpus 0.6.56, 2026-09-24 (round 26 CLOSED on both gates, `GO`/`GO`, on `df91ae7` from the real test)

| | |
|---|---|
| our released version | **0.6.56**, released 2026-09-24 (pre-release, as all `v0.*` are) — **the release both round-26 closing laps name**, with no round open, so no override. It is the first release that installs `df91ae7` by default, and it carries what the real test found in us: a killed rip explained, finished tracks marked Done again, and one folder per acceptance run. 0.6.55 installs `3e01bb3`. |
| ripper we **pin** | **`df91ae7`** in our code on `main` — `cyanrip 0.9.4-rc2+platterpus.15`, `release_seq` 25 — **rolled when round 26 closed on our gate, 2026-09-24**, in the commit that releases our lap 5, as that lap said it would. Users get it from **0.6.56**, which goes out after your lap 6 and your `.16`; 0.6.55 as released installs `3e01bb3`. Both values derived from your tree rather than from your lap: `meson.build` at the pin declares the version (line 21), `meson_options.txt` is byte-identical to `3e01bb3`'s, and `release-manifest.json` names `df91ae7` on both channels. |
| approved by | **round 26**, for Platterpus **0.6.55** — read off your round 26 lap 4's `HANDSHAKE-APP-VERSION` (sha256 `7a56b1d2…`). **The first pin approved on the round's own hardware test**: the acceptance run on 0.6.55 with `df91ae7` installed, 258 of 261, whose three failures were one rip killed when its container was stopped from outside both programs. The bundle is in both trees (ours: `docs/handshake/artifactsround26/`; yours: `docs/rig-2026-09-24-df91ae7/`). |
| pin **under review** | **`df91ae7`**, the same as the pin: round 26 closed on it, so nothing is under review until your round 27 opens on `.16`. |
...
| **what our own app says** | `a_round_is_reviewing_a_build()` is **`False`**, correctly: the reviewed pin and the approved pin now coincide. |
| rounds 1–26 | **all closed, bilateral `GO`, on both gates.** Round 26 closed at six laps: your lap 4 `GO` from the real test, our lap 5 `GO` from our reading of the same bundle, and your lap 6 transcribing ours. You published `.16` at `221a1df` (`release_seq` 26, both channels) after it; round 27 reviews `.16`. |
