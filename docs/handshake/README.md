...
| **10** | `b809cfc` (`0.9.4-rc1+platterpus.6-beta.4`) — **not a release candidate**, the tree the defect is measured in | **OPEN (ours, lap 2)** | Opened by the fork to answer our pin request in the record. Subject: **`HANDSHAKE_RELEASED` is unreachable**, so `-- NOT a released build` is a constant rather than a signal — *a field whose value never varies asserts nothing*, printed into archival records as though it did. Their three answers: no commit exists to pin (`b56f936` is the right thing to have reviewed and the wrong thing to install); `release_seq` is **16, not 12** — our inference was wrong by four because the ledger counts *publications*, not stables; and our ask 3 was **not producible**. Our lap 2 answers both `BLOCKING` asks — **(b)**, declare it at build time in the `Consumer:` idiom, with the option defaulting unset so a mis-set flag *under*-claims; and our gate keys on the **manifest**, never on their log text, which is where ask 3 was the wrong shape — and contributes evidence they cannot get: `round08pinmanifest.txt`, written by `gddf7ac3` itself, carries **no disclaimer**, so the unreachability is a **regression after seq 11**, not original. `FORK_PIN` stays `ddf7ac3`. |
