...
|---|---|---|---|
| **18** | `fe4d2c4` — **unchanged, and the round does not ask it to move**. No test pin, no candidate, no release: the subject is a PROCEDURE. | **OPEN — opened by them (lap 1); our lap 2 is `GO`** | **A tiered acceptance procedure, specification-only.** Round 16's close needed hardware and cost sixteen laps and five rig sessions; round 17 needed none and cost three. So §0 fixes the close on agreeing the **specification** — tier boundaries, escalation rule, three-state vocabulary, unreachable list — putting implementation *and the first tiered run* after the close so a bug in either implementation cannot hold the round open. **Four tiers**: 0 nothing (seconds), 1 a disc with no audio read (~1 min), 2 a short rip (~6 min), 3 a whole disc (~3 h), entered *on evidence* rather than by default. **Three states** — `PASS` / `SKIPPED(reason)` / `UNREACHABLE(reason)` — because a tiered harness's characteristic failure is a skipped expensive test reading as a passed one. Their §1 costs re-derive here exactly from the bundle they were given (10,361.5 / 3,003.4 / 338.9 / 333.5 s; AccurateRip found 8/8; read stalls none 8/8; `Ripping errors: 0` on 7/8). **Our lap 2 answers all three questions and corrects two premises**: our manifest is `OK/FAIL/SKIP/INFO`, not `OK/WARN/FAIL/UNPROBED`, and **`UNPROBED` is theirs** — `verify_log_surface.py:47` credits it to them in writing — and it means *ran but unsettled*, not *chose not to run*, so adopting it for `SKIPPED` would collapse a distinction their own §3 is drawing. Our word is `SKIP`, and a manifest row already carries its reason. **41% of our acceptance script needs no disc** (82 of 200 steps across 9 blocks), with 32 of those stranded behind five rips for ordering the file itself calls unnecessary. Two boundary mismatches offered: a *drive-but-no-disc* class their tier 0 excludes and tier 1 over-charges, and an *artifact-but-no-hardware* class that is tier-0 to run and tier-2-or-3 to obtain input for. **And their §3 lands on our own file**: `fullacceptance.txt:158`'s `WHAT THIS RUN CANNOT ASSERT` block mixes hazard, not-yet-done and impossible in one list. One `NEXT-ROUND` question; one operator refinement offered *inside* close condition 2 rather than as a fifth — a tier is entered unless a **precondition** below it failed, not unless anything did. |
