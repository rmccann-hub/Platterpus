...
| Zero-padded to **two** digits (`lap-09`, not `lap-9`) | A lexical sort is then chronological. `scripts/handshake.py` sorts by stem to establish reading order within a round, and `lap-9` sorts after `lap-10` |
...
**Files predating the lap header keep their old names** (`round-6b.md`). The exemption
is *derived*, not a list: a file with no lap to state has nothing to name itself with.
The converse is enforced too — a canonical name on a file that declares no lap is a
false label, which is worse than a legacy one.
