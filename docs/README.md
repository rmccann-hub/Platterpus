...
| [`mp3-wav-support.md`](mp3-wav-support.md) | **Research + design (P1, pre-build):** adding MP3/WAV output — per-format parity semantics, verified encoder facts (whipper FLAC-only → post-rip re-encode; cyanrip native `-o`; the LAME `-q4`/#516 CBR-only bug; WAV tag/art limits), the design routing it through the dependency subsystem + backend abstraction, and the decision gates the build is waiting on. Critical Rule #4 (FLAC-only for v1) still holds. |
