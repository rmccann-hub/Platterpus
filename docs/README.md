...
| [`release-signing.md`](release-signing.md) | **Maintainer how-to (2026-07-21):** the offline-key `minisign` release-signing ritual — why it's offline (a CI-held key adds nothing over the SLSA attestation), one-time keygen + baking the public key into `update_signing.PUBLIC_KEY_B64` to *arm* the fail-closed gate, the transition it forces (the first armed release must carry a `.minisig`, and all after it), and per-release sign/verify/upload steps. The verify side (`src/platterpus/update_signing.py`, ships dormant) + KDD-26 are the code/decision companions. |
