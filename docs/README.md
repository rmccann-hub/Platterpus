...
| [`eac-tracker-requirements-2026-07.md`](eac-tracker-requirements-2026-07.md) | **External-standard comparison (2026-07-29):** what EAC settings and log contents tracker logcheckers actually require, measured against what Platterpus does, row by row, with sources. The headline: a cyanrip log can **never** pass — the gate is ripper *identity*, checked before any quality line, so closing every quality gap would move the score from unscoreable to unscoreable. Records that whipper's accepted checksum is a plain SHA-256 (the same scheme as ours — identity is the only wall), corrects an earlier claim in the feasibility doc, and ranks the quality rows still worth earning. Also records a change **attempted and reverted**: asserting `Make use of C2 pointers : No` on a secondary source, which a fabrication test correctly blocked. |
...
| [`../PLANNING.md`](../PLANNING.md) | Architecture, directory tree, per-module responsibilities, adapter designs, dependency-manager design, keyed design decisions (KDD-01 … KDD-33) |
