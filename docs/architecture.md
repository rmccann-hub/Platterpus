...
#### Moved from `CLAUDE.md` (2026-09-26): the Qt / PySide6 threading rules in full

*Verbatim from `CLAUDE.md` at `f5305a7` (Critical rules, rule #9), moved here when that file was trimmed to its rules and pointers. Inside it, “here” and “this file” mean `CLAUDE.md`, and “above” / “below” mean the neighbouring entries of that section.*

**Qt / PySide6 threading rules.** Every one of these was paid for with a crash or a freeze; none is theoretical.
...
- **That applies to EVERY class that creates a thread, not just the window** — and it is swept, not trusted: `tests/test_qthread_ownership.py` derives every `self.x = QThread(...)` owner from the source, resolves responsibility through the MRO (a mixin's thread is the concrete class's job), and requires the stop to be *reachable* from a teardown hook. It also forces a flag-only `cancel()` to be justified in an allowlist, and ratchets the workers that have no `cancel()` at all — **five when the rule was written (2026-07-29), three today** — and that list may shrink, never grow. Read the live count from `tests/test_qthread_ownership.py::_WORKERS_WITHOUT_CANCEL` rather than this sentence; a number written here decays one fix later. The rule existed and was enforced for one class out of nine; the dialog with no teardown at all was found by a person reading code (see `docs/testing.md` §5.o — enforce a rule across the codebase, not at the place it was learned).

#### Moved from `CLAUDE.md` (2026-09-26): Never block the GUI thread — the convention as stated there

*Verbatim from `CLAUDE.md` at `f5305a7` (Code conventions, the GUI-thread bullet), moved here when that file was trimmed to its rules and pointers. Inside it, “here” and “this file” mean `CLAUDE.md`, and “above” / “below” mean the neighbouring entries of that section.*
...
#### Moved from `CLAUDE.md` (2026-09-26): A new testing capability is a SCRIPT VERB — the convention in full

*Verbatim from `CLAUDE.md` at `f5305a7` (Code conventions, the script-verb bullet), moved here when that file was trimmed to its rules and pointers. Inside it, “here” and “this file” mean `CLAUDE.md`, and “above” / “below” mean the neighbouring entries of that section.*
...
#### Moved from `CLAUDE.md` (2026-09-26): Did I check the preconditions where the thing HAPPENS, or where it was scheduled?
...
### 6.3 Why each CI and release step exists (moved from `CLAUDE.md` *CI / release*, 2026-09-26)

`CLAUDE.md` owns the operational steps of a release and keeps them. The reasons behind each step, and the incidents that produced them, used to sit inline beside the steps; they moved here verbatim when that file was trimmed to its rules and pointers, so a release run from `CLAUDE.md` can still find why a step is there.
