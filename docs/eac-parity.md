...
| Frame-450 ("offset-variant") match | (not distinctly labelled) | `Accurip 450: BF62B1DA (..., track is partially accurately ripped)` | **cyanrip extra.** The checksum of **one frame** (frame 450, six seconds in), printed only after both whole-track checksums missed (`cyanrip@df91ae7:src/checksums.h:74-78`, `src/cyanrip_log.c:594`). It is not a pressing: a shifted pressing moves frame 450 too, and a submitted pressing matches its own whole-track entry. Surfaced as "partially accurate", never as a verified match. Our screens and report say "only one frame matched" since 2026-09-24; our EAC-compatible log keeps its wording until round 27 (round 7 lap 11, H4). |
