...
| Log layout (side-by-side comparable) | **Present (2026-07-27)** | Checked against a genuine EAC V1.8 log of the same disc: the TOC table is byte-identical, and the archival header, output-format block and end-of-rip status report use EAC's rows and wording. Unreportable rows are labelled `(not reported by cyanrip)`. Attribution + checksum stay deliberately distinct — layout is parity, provenance would be forgery. |
