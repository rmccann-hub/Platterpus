...
> **Status: v0.6.46 — out of beta.** Handshake rounds 1 through **16** are all closed with `GO` from both projects. **Round 16 closed on a joint hardware session**: the fork's Run A on pin `a9aedf0` (tested as `ddc1e8c`, the same program — `git diff a9aedf0..ddc1e8c -- src/ meson.build` is empty) settled all three of its close conditions with `0 FAIL, 0 UNPROBED`, including **`-H` with de-emphasis on a drive for the first time in twelve rig sessions**, and our own `verify_log_surface.py` accounted for every one of **1,055** log lines across its five logs with **0 unaccounted**. Both sides' pre-commits resolved on an exit code, so neither had to argue a verdict. **The approved pin is not the installed one, and that is not a gap.** This build pins cyanrip `978f9b0`; the fork has published neither `a9aedf0` nor `ddc1e8c` as numbered releases, so the in-app update offer — which reads their release manifest — cannot reach either, and the production pin rolls when they publish rather than when a round closes. `Help → Install a cyanrip build…` and `--install-ripper <commit>` are the routes that do not wait for a release. **What 0.6.46 carries**: two fixes to the one archival step that grades a cancelled rip's record, a clause-2 verdict rule that was arithmetically impossible to satisfy and so could only ever have accused the fork, one deletable `~/platterpus-rig` instead of a folder per run in `$HOME`, and a pin-field guard the fork's own gate caught us needing. **Nothing is published yet**: the next round is the release round and approves two releases as a pair — the fork's and ours, together — so this version exists and is named before it ships.
>
