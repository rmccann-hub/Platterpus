...
> **Status: v0.6.27 — out of beta.** The ripper pairing is **jointly verified**: handshake rounds 8 through 13 are all closed with `GO` from both projects, and round 14 is open with a single close condition — one hardware acceptance pass on the released pair. This build still installs cyanrip `0.9.4-rc1+platterpus.5` (`ddf7ac3`), the build round 8 approved and rig-tested on real hardware. **A reviewed pin is not an installed one**, and a pin we have not run on a drive is a pin we do not claim — so the pin moves when round 14's acceptance pass closes it, not before. If you want the build under review now, tick the ripper **beta** channel in Settings and take the offer; your rip reports will correctly say `unapproved` until that round closes.
>
...
>
> **v0.6.27 makes the acceptance run re-runnable.** Every album name in the acceptance script now carries the run's own timestamp, so a second run cannot land on the first run's folders. Before that, a re-run raised *"Album already ripped"* on every rip and the script answers that prompt in exactly one place — which cost the 2026-08-25 attempt sixteen of its seventeen failures and the whole of its final section. **v0.6.26 is what the overnight acceptance run found.** The run passed 209 of 212 steps on real hardware — a complete 14-of-14 disc rip, the `-T unicode` path confirmed end to end, MP3/WavPack/WAV all derived, cancel and drive recovery both working — and then lost its final section to four defects of ours lining up at once. A `wait-for-rip` asked for six hours against a three-hour cap and **waited zero**: an over-generous bound became no bound at all, so the batch ran on against a live drive, a cache probe opened the same device 1.2 seconds later, and the unattended-quit helper declared the run finished and killed the reader at 1.48% of track 1. All four are fixed, each with a regression test proved non-vacuous. The run also produced the measurement two projects had been arguing about from first principles: a track that genuinely re-read gave **four different ratios across four paranoia counters**, so the tempting `disc == passes × sum` is refuted on real media while the inequality holds everywhere.
>
