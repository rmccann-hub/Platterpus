...
> **Status: v0.6.38 — out of beta.** Handshake rounds 1 through 14 are all closed with `GO` from both projects. **Round 15 is OPEN**: its subject is cyanrip `0.9.4-rc2+platterpus.11` (`978f9b0`), the fork has declared `GO`, and the one remaining close condition is a hardware acceptance pass — ours to run. **The app half moved once more, to `0.6.38`, and the fork is being told so in the same round** — lap 7's F1 commitment is a promise about *disclosing* a move, not about stability, and this is that disclosure. What moved: four ARCHIVAL acceptance checks could be satisfied by finding nothing, so a run on `0.6.37` would have produced a green transcript over three sections that asserted nothing (§I's completion footer, §N's secure re-read, §E's disc identification) and 22 unfailable snapshots. A pass proving less than it appears to is worse than no pass, and the run is eight hours. It took three attempts to get there and the record should say so plainly: `0.6.33` demanded a ripper build its own dialog refused to install; `0.6.34` budgeted three hours for a six-hour whole-disc secure re-read, so the run's archival section produced no evidence at all; `0.6.35` fixed both, and reading that run's bundle then found two defects in the *record* it produces — a clean rip reporting thirteen errors, and a diagnostics header naming the approved cyanrip build rather than the one that ran. An acceptance pass exists to produce trustworthy evidence, so a build that mis-describes its own results is not one to run it on. The **cyanrip** pin under review has not moved through any of that and does not move mid-round. Until the round closes, a rip made with `978f9b0` correctly reports `unapproved`; that is the record being honest, not a defect. This build installs cyanrip `0.9.4-rc2+platterpus.10` (`d9c058c`) — the build round 14 approved, and the first production pin that was already a published release when it was reviewed. Its evidence is a whole-disc uniform secure re-read on real hardware: `-Z 2 -r 3` at paranoia maximum, all 14 tracks converged *2 out of 2 matches*, `Ripping errors: 0`, and the completion footer intact. **A reviewed pin is not an installed one**, and a pin we have not run on a drive is a pin we do not claim — so the pin moves when a round closes on hardware evidence, not before. Rips with this build report `approved`. The fork has since published `0.9.4-rc2+platterpus.11` (`978f9b0`); that build is **newer but unreviewed**, so installing it correctly reports `unapproved` until a round closes on it.
>
