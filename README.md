...
> **Status: v0.6.45 — out of beta.** Handshake rounds 1 through 15 are all closed with `GO` from both projects. **Round 16 is OPEN**, opened by the cyanrip fork on pin `a9aedf0` (`0.9.4-rc2+platterpus.11`), with `ddc1e8c` agreed by both sides as the test pin for the joint hardware session — the same program, verified: `git diff a9aedf0..ddc1e8c -- src/ meson.build` is empty. **Its one close condition is a hardware acceptance run on that pin**, fixed at their lap 1 under S-13 so it cannot grow: the AccurateRip path still succeeding with their rewritten response parser, `-H` together with de-emphasis producing correct de-emphasised audio, and no line we parse having moved except the ones their §D names. Four log-format changes are announced and accounted for — two timestamps gain a UTC offset, two footer lines appear on paths that previously emitted none, `Preemphasis:`/`FLAGS PRE` now describe the audio rather than the setting, and the `-j` record moves to schema `cyanrip-diagnostics/4`. **That last one is NOT a no-op here, and this banner said it was until v0.6.42.** We do read the `-j` record: `rig_check` takes one field, `invocation`, out of it to check that the argv we compose is the argv the binary receives. Nothing on our side gates it by schema — `SUPPORTED_SCHEMAS` is over the fork's *release manifest* — so a `/4` record cannot be refused; the additive fields are ignored, and only a rename of `invocation` would bite, loudly. **v0.6.43 carried the fixes for the fork's two lap-4 findings**, which `0.6.42` shipped without — the `[plan]` block denying a flag every rip argv carries, and `parser/interrupted` giving one verdict for two opposite states. **v0.6.44 adds two fixes to the FAILURE path**: the abort message named the reviewed pin where the round's agreed build is the test pin, and a run that stopped before touching the drive still packed an evidence archive and offered to open its folder. Neither changes a rip. **v0.6.45 makes the test-pin install a menu item** — `Help → Install a cyanrip build…`, with the build an open round needs already selected, because the in-app update check reads the fork's release manifest and so cannot offer a commit they nominated without releasing. **The reviewed pin is not the installed one.** This build still pins cyanrip `978f9b0`, the build round 15 closed on — a pin moves when a round closes on hardware evidence, not before — so rips on `978f9b0` report `approved` and rips on the reviewed or test build report `unapproved`, which is correct in every direction. **The fork has not published `a9aedf0` or `ddc1e8c` as numbered releases**, so the in-app update offer cannot reach either; `--install-ripper <commit>` can, and closing that gap for a GUI-only operator is tracked as ours.
>
...
`cyanrip 0.9.4-rc2+platterpus.11 (platterpus-fork-g978f9b0)`. The parenthetical is
