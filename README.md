...
> **Status: v0.6.24 — out of beta.** The ripper pairing is **jointly verified**: handshake rounds 8 through 13 are all closed with `GO` from both projects. Round 13 was the endgame round and it did what a handshake is for — it caught a flag we had pinned backwards hours earlier, and our own finding turned out to be a real defect in the ripper that had survived five rounds of checking. This build still installs cyanrip `0.9.4-rc1+platterpus.5` (`ddf7ac3`), the build round 8 approved and rig-tested on real hardware. **A reviewed pin is not an installed one**, and a pin we have not run on a drive is a pin we do not claim — so the pin moves when the next hardware acceptance pass closes round 14, not before.
>
...
>
> **v0.6.24 is what a full hardware acceptance run found, and almost all of it was invisible in the pass/fail line.** The run reported four failures; reading its *artifacts* found nine defects. **A completed 14-track archival rip was silently overwritten by a 2-track one** — the guard that should have asked predicted where the rip would land and missed by one character, so it probed a folder that did not exist. It no longer predicts: it reads what is on disk. The unattended run gave its post-rip verification a grace period of **zero seconds** and then left an archival record that said `"No errors occurred"` while missing its cover art, its CTDB verdict and its checksums. Post-rip checks kept reading a folder the *next* rip was overwriting and logged a corruption error about a file that was merely mid-write. An EAC-compatible log said `2 track(s) accurately ripped` and then, two lines later, `Some tracks could not be verified as accurate` — both true-looking, one false. AccurateRip now shows the database's best confidence beside your track's own — `OK (3 of 200)` — because a bare number cannot tell you the one thing that changes what you should do: that you have a different **pressing**, not a bad rip.
>
