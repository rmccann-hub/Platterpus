...
> **Status: v0.6.52 — out of beta.** Handshake rounds **1 through 23** are all closed with `GO` from both projects. The approved pair is cyanrip `0.9.4-rc2+platterpus.13` at **`2cce60d`** and Platterpus **`0.6.52`**, approved by **round 23** — which reviewed that same commit a second time for a property round 22 did not examine: its behaviour on a drive. Reviewing one commit twice for two properties is not a stalled pin. **The substance of 0.6.52 is three defects a real user hit on a fresh install, two of which were one bug.** The launch-time dependency probe finishes inside the first-run *"Set up Platterpus?"* question's nested event loop and its result is a queued slot, so Qt delivered it there and opened a second setup wizard behind the first — two build pipelines running `git`, `meson`, `ninja` and `sudo install` against one container, which is why the install failed and why the dialogs doubled. Every wizard now opens through one chokepoint. Separately, uninstalling removed the menu entries but never rebuilt the menu cache that install refreshes, so the launcher kept a Platterpus item pointing at a deleted AppImage — which reads as a broken install rather than a finished uninstall. And setup, dependencies and both update checks are now one window, **Tools → Setup & Updates…**, replacing six menu items spread across two menus plus a check that was reachable only from inside Settings.
>
