...
> **Status: v0.6.53 — out of beta.** Handshake rounds **1 through 25** are all closed with `GO` from both projects. The approved pair is cyanrip `0.9.4-rc2+platterpus.14` at **`3e01bb3`** and Platterpus **`0.6.53`**, approved by round 24 and re-approved by **round 25**, which settled the shared protocol text (v6). **Round 26 is open** on the fork's new `+platterpus.15` (`df91ae7`), and it closes on a real hardware test of `.15` installed through this app. **`3e01bb3` ships in 0.6.54**, and 0.6.54 is also the release whose acceptance test demands `.15` and offers to install it; 0.6.53 as released still installs round 23's pin, `2cce60d`, and a rip made with `.14` on 0.6.53 is stamped `unapproved` until you update — the gap round 24 exists to close. A rip on `.15` is stamped `unapproved` too, correctly, until round 26 closes on it. `.14` changes log text only: each track now reads *"read successfully"* rather than *"ripped and encoded successfully"*, and a new `Encoder errors:` line reports the encoder separately. **The substance of 0.6.53 is what a full acceptance run found — and none of it was a failing test.** The run reached 247 of 247 with zero errors, and four things turned out to be unable to fail. The section that rips the whole disc switches CTDB and FLAC-integrity verification on and then checked that the *settings* round-tripped, so when the next section's rip dropped both checks unfinished — on three of eight rips — nothing noticed; a new `expect-verification` step now grades what the checks actually left. Every track of an archival rip was flagged as needing *"unusually heavy re-reading"*, because two agreeing checksums cost three reads and the threshold was three: the flag was measuring the setting, not the disc. The Diagnostics dialog had never once shown a dependency, on any machine, while the rip report beside it listed all seven. And this page claimed a full-green hardware pass the project's own ledger denies. Each of the four now has a check that fails when it recurs.
>
...
>
> **Latest hardware — the most complete run yet, and still not a full pass.** On **2026-09-22** the acceptance run on the Pioneer BDR-209D under Bazzite reached the last step at **247 of 247, zero failures**, on app `0.6.52` against ripper `2cce60d` — 4h14m, eight rips, zero errors and zero tracebacks across 132,204 log lines. It is recorded `partial`. **The field-evidence ledger carries seven rows and no `full-green` row at all**, and that is the number to read rather than any step count. Three of the eight rips had their post-rip CTDB and FLAC-integrity checks dropped unfinished when the next section started ripping — including section F, the whole-disc archival rip, which switches both checks on and whose record duly says *"an absent result is not a passed one"* — and no step read that, so the run could not fail over it. **A count of passing steps measures the script.** The 2026-09-12 run was recorded `full-green` and was re-graded `partial` on the maintainer's ruling for the same class of reason: two archival sections passed while the derived-format files they exist to prove were never written. Both gaps now have assertions and a sweep that requires the script to carry them. The next minor, **`0.7.100`**, is gated on a full hardware pass and **has not met it**; `0.9.1` needs **two** full-green passes across **at least two machines and two distros** (maintainer ruling, 2026-09-13) — one rig passing twice answers *was it luck* and says nothing about *is it green only because of this machine*. A gate refuses a version bump the ledger does not support.
>
...
- [`PLANNING.md`](PLANNING.md) — architecture, directory tree, per-module responsibilities, keyed design decisions (KDD-01 through KDD-36)
