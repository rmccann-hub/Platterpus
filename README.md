...
> **Status: v0.6.21 — out of beta.** The ripper pairing is **jointly verified**: handshake rounds 8 through 11 are all closed with `GO` from both projects, and every number either side declared reproduces on the other's tree. This build installs cyanrip `0.9.4-rc1+platterpus.5` (`ddf7ac3`), the build round 8 approved and rig-tested on real hardware — `Ripping errors: 0`, banner verified before and after the rip. **Both of the things this line used to call unproven have now been measured on the rig, and one of them found a defect:** the fork's `-x` **cache probe** ran for the first time on 2026-08-19 (`32 sectors, 73.5 KiB, uncached read 362.6 ms`) and then went on to **rip the whole disc**, which is a fix owed by the fork rather than a gap here; and the **drive-open fix is verified** as of 2026-08-20 — after a mid-rip cancel the drive re-scanned and re-identified the disc, which a held reader cannot do. What is still unproven is a *completed* second rip after a cancel. *(This sentence said "`-x` force-overread" until 2026-08-18, conflating two different flags — overread is `-O`, it **has** run on the BDR-209D, and it **hung the drive ~23 minutes**; see `docs/dependency-contracts.md`. Reaching for the overread toggle on that drive because of this line would have cost a session.)* Those are what a v1.0 would need; this is not that release, and it does not pretend to be. **v0.6.21 is a hardening release with no user-visible feature change**, from an enforcement audit of the project's own rules: three message boxes were rendering external text (an exception message, a MusicBrainz-derived folder name, a drive diagnosis) as HTML, so a `<` in any of them silently swallowed the rest of the line — worst of all in the crash dialog people screenshot to report bugs. Both guards against committing copyrighted media were fail-open on a producer error, and shared the single idiom that made them so. Neither had a test.
>
