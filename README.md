...
> **Status: v0.6.40 — out of beta.** Handshake rounds 1 through 14 are all closed with `GO` from both projects. **Round 15 is CLOSED**, `GO` from both projects, on cyanrip `0.9.4-rc2+platterpus.11` (`978f9b0`) paired with Platterpus `0.6.37` — so every handshake round to date is closed. Its close condition was a hardware acceptance pass, and the eight-rip run of 2026-09-05 met it: the fork verified it from the artifacts rather than from our pass count, checking every one of the eight logs for the pin banner, `Ripping errors: 0`, `Rip completed: yes` and `cyanrip -Y` exit 0. **Round 16 is theirs to open.** **The app half moved once more, to `0.6.38`, and the fork is being told so in the same round** — lap 7's F1 commitment is a promise about *disclosing* a move, not about stability, and this is that disclosure. What moved: four ARCHIVAL acceptance checks could be satisfied by finding nothing, so a run on `0.6.37` would have produced a green transcript over three sections that asserted nothing (§I's completion footer, §N's secure re-read, §E's disc identification) and 22 unfailable snapshots. A pass proving less than it appears to is worse than no pass, and the run is eight hours. It took three attempts to get there and the record should say so plainly: `0.6.33` demanded a ripper build its own dialog refused to install; `0.6.34` budgeted three hours for a six-hour whole-disc secure re-read, so the run's archival section produced no evidence at all; `0.6.35` fixed both, and reading that run's bundle then found two defects in the *record* it produces — a clean rip reporting thirteen errors, and a diagnostics header naming the approved cyanrip build rather than the one that ran. An acceptance pass exists to produce trustworthy evidence, so a build that mis-describes its own results is not one to run it on. The **cyanrip** pin under review has not moved through any of that and does not move mid-round.  This build installs cyanrip `0.9.4-rc2+platterpus.11` (`978f9b0`), rolled forward from `d9c058c` when round 15 closed. **A reviewed pin is not an installed one**, and a pin we have not run on a drive is a pin we do not claim — so the pin moves when a round closes on hardware evidence, not before, and that is what happened here. Rips with this build report `approved`. The pin did not move once during round 15, which matters for the evidence: the acceptance bundle that closed the round was produced by this exact binary, so the artifact and the approval describe the same build.
>
