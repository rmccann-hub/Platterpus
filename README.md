...
> **Status: v0.6.46 — out of beta.** Handshake rounds 1 through **16** are all closed with `GO` from both projects. Round 16 closed on a joint hardware session: the fork's Run A on pin `a9aedf0` (tested as `ddc1e8c`, the same program) settled all three of its close conditions with `0 FAIL, 0 UNPROBED`, including **`-H` with de-emphasis on a drive for the first time**, and our own `verify_log_surface.py` accounted for every one of 1,055 log lines across its five logs. **Round 17 is OPEN, and it is the release round**: it approves *two* releases as a pair — cyanrip `0.9.4-rc2+platterpus.12` at `fe4d2c4` and Platterpus `0.6.46` — to be published together and tested together, with **neither publishing alone**. Its close condition is fixed at their lap 1 under S-13 and needs no hardware: both sides name a candidate by SHA, each verifies the other's declared observable surface, and the close authorises publication — the release test follows the close rather than sitting inside it. **Three announced changes cross the published build** (`978f9b0`), and each is handled here: the two record timestamps gained a `±HH:MM` offset, which our EAC export reads with `fromisoformat` *first* because it is the only parse that can see a zone; the `-j` record moved to schema `cyanrip-diagnostics/4`, which we never gate on — `SUPPORTED_SCHEMAS` is over the fork's *release manifest*, not that record — and `rig_check` takes only `invocation` from it; and the P5 string `Error parsing string: %s!` was removed, which our message inventory deliberately retains because the build we still ship prints it. **The approved pin is not yet the installed one.** This build pins cyanrip `978f9b0`; the fork has published neither `a9aedf0` nor `fe4d2c4`, so the in-app update offer cannot reach either and the production pin rolls when they publish, not when a round closes. `Help → Install a cyanrip build…` and `--install-ripper <commit>` are the routes that do not wait for a release. **What 0.6.46 carries**: two fixes to the one archival step that grades a cancelled rip's record, a clause-2 verdict rule that could only ever have accused the fork, one deletable `~/platterpus-rig` instead of a folder per run in `$HOME`, and a pin-field guard the fork's own gate caught us needing.
>
