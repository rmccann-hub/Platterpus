...
> **Status: v0.6.48 — out of beta.** Handshake rounds **1 through 19** are all closed with `GO` from both projects. The approved pair is cyanrip `0.9.4-rc2+platterpus.12` at **`fe4d2c4`** and Platterpus **`0.6.47`** — the fork has published theirs to both channels, so the approved build, the reviewed build and the build you can actually install are one object. **Rounds 17, 18 and 19 each closed in three laps**, on the same pin, after a round 7 that took 37. Round 19 is the first whose approval rests on a provider contract **regenerated from the pin itself**: the fork shipped `PROVIDER-CONTRACT.md` at `g7b2fda6`, our fatal-message inventory rebuilds from it byte-identically at 120 P5 + 7 P5a, and the argv flag table we check every invocation against is the current round's own rather than three rounds old. **The substance of 0.6.48 is the acceptance script**: the whole-disc secure re-read — the accuracy claim itself — never asserted the output format it writes in, inheriting it from a preset field that nothing checked.
>
