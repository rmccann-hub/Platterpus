...
>
> **v0.6.25 is the release the overnight acceptance run is built for.** It adds the two things that run needed and could not say: `expect-refused`, which asserts a settings guard **refuses** a bad value *and does not write it anyway* — input validation was institutional here and none of it was reachable from a test script — and `expect-ripper-under-review`, which removed the last hardcoded cyanrip build tag from the acceptance script. That tag had gone stale three times in two days: the ripper project published three betas on the channel this app's own installer resolves, so anyone following our instructions installed the build we sent them to and was told, four seconds in, that it was the wrong one. Changing the literal was never the fix. The acceptance script is now end to end — 212 steps covering validation, every dialog, two whole-disc rips, the overwrite prompt, a mid-track cancel and drive recovery, **MP3, WavPack and WAV** (the derived formats nothing had ever tested on hardware), the goal presets, and the probe-only cache invocation.
>
