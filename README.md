...
> **Status: v0.6.50 — out of beta.** Handshake rounds **1 through 20** are all closed with `GO` from both projects. The approved pair is cyanrip `0.9.4-rc2+platterpus.12` at **`fe4d2c4`** and Platterpus **`0.6.49`** — the version the round reviewed, which is what that field names rather than the newest that exists. **The substance of 0.6.50 is the state-ownership fix behind three separate defects in two days.** Every per-album fact a rip report is built from used to live on the window, whose lifetime is *the current rip* — so those facts moved out from under their readers the moment the next rip began. That one shape produced a report describing a configuration its rip never ran under, a post-rip chain abandoned so that no MP3 or WavPack was written at all, and a chain that *succeeded* and had every result discarded 655 ms later. An album now owns its own record, so a late CTDB, FLAC-integrity, derived-format, transcode or checksum result is written into the report of the album it describes. Round 20 also closed `GO`/`GO` on two procedure items, one of which was a defect the fork found in code of ours written the same day.
>
