...
> **Status: v0.6.49 — out of beta.** Handshake rounds **1 through 19** are all closed with `GO` from both projects. The approved pair is cyanrip `0.9.4-rc2+platterpus.12` at **`fe4d2c4`** and Platterpus **`0.6.47`** — the fork has published theirs to both channels, so the approved build, the reviewed build and the build you can actually install are one object. **The substance of 0.6.49 is a hardware run that passed and should not have.** The 2026-09-15 acceptance pass reported 241 of 241 steps green on the approved pair and wrote **no MP3 and no WavPack file at all**: a post-rip guard meant to discard a finished rip's *result* when the next rip starts was discarding its remaining *work* too, and the transcode runs last. The two acceptance sections that exist to prove those outputs are graded ARCHIVAL and could not fail — both asserted against the ripper's own log, and the ripper is always invoked `-o flac`. Fixed, along with a report whose `verification.gates` said a check `"ran"` beside a null result on five of eight rips.
>
