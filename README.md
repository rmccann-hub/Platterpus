...
> **Status: v0.6.51 — out of beta.** Handshake rounds **1 through 21** are all closed with `GO` from both projects. The approved pair is cyanrip `0.9.4-rc2+platterpus.12` at **`fe4d2c4`** and Platterpus **`0.6.50`** — the version the round reviewed, which is what that field names rather than the newest that exists. **The substance of 0.6.51 is that an acceptance run can no longer be graded against the wrong ripper build.** On 2026-09-17 a full hardware session reported 247 of 247 steps green and established nothing: it ran on the cyanrip *release* pin while round 21's close condition was about the *test* pin, a different program carrying both of that round's breaking log changes. The guard for exactly this existed, was called, and passed — it had been widened in round 16 on the measured grounds that the two pins were then the same program, and round 21 was the first round where that was false. It now accepts both only while they are the same program, names the required build in its refusal, and the evidence bundle records which build the session was *for* rather than only which one ran. Round 21 closed `GO`/`GO` on the first approval of `fe4d2c4` backed by hardware on the round's own subject.
>
