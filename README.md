...
> **Status: v0.6.41 — out of beta.** Handshake rounds 1 through 15 are all closed with `GO` from both projects. **Round 16 is OPEN**, opened by the cyanrip fork on pin `a9aedf0` (`0.9.4-rc2+platterpus.11`), which carries all seven items their round-15 lap 14 held. **Its one close condition is a hardware acceptance run on that pin**, fixed at their lap 1 under S-13 so it cannot grow: the AccurateRip path still succeeding with their rewritten response parser, `-H` together with de-emphasis producing correct de-emphasised audio, and no line we parse having moved except the ones their §D names. Four log-format changes are announced and accounted for — two timestamps gain a UTC offset, two footer lines appear on paths that previously emitted none, `Preemphasis:`/`FLAGS PRE` now describe the audio rather than the setting, and the `-j` record moves to schema `cyanrip-diagnostics/4` (a no-op here: nothing reads that file). **The reviewed pin is not the installed one.** This build still installs cyanrip `0.9.4-rc2+platterpus.11` (`978f9b0`), the build round 15 closed on — a pin moves when a round closes on hardware evidence, not before, so rips on `978f9b0` report `approved` and rips on the reviewed build report `unapproved`, which is correct in both directions. **The fork has not published `a9aedf0` as a numbered release**, so the in-app update offer cannot reach it; `--install-ripper a9aedf0` can, and closing that gap for a GUI-only operator is tracked as ours.
>
