...
| ffmpeg | Host/container, **optional (P1)** — only used to transcode whipper's FLAC output to MP3/WAV (whipper is FLAC-only; the cyanrip backend encodes those natively and needs nothing extra). Registered in `deps/registry.py`; absent only disables the MP3/WAV transcode. Already present wherever cyanrip is (cyanrip is built on FFmpeg). Groundwork landed 2026-06-23; not yet user-exposed (Critical Rule #4 — FLAC-only for v1). | `>=4.0` | Active (FFmpeg project) | — (LGPL/GPL build; invoked as a subprocess, never linked) |
