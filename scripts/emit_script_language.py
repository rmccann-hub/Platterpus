...
def _document() -> str:
    ...
    add("")
    add("Arguments containing a control character or a line break are refused:")
    add("NUL, newline and every other C0 control, DEL, the C1 range (which holds NEL)")
    add("and U+2028/U+2029. That is not injection — no shell is involved — it is")
    add("**log forgery**: cyanrip writes its argv into an archival log, and any of")
    add("these could fabricate a line in a document whose whole purpose is being")
    add("trustworthy evidence.")
    add("")
