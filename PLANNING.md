...
- **Hardware-gated finalization.** The code + tests ship now; the real-hardware step (validate `-A` output wording on the BDR-209D → tune the signal phrases → add a real-output regression fixture, and confirm `/usr/bin/cd-paranoia` provides-install + export succeed in the container) is the maintainer's rig session — the same code-side-prep-then-hardware pattern as the offset and INDEX-00 work.
