...
└── src/
    └── whipper_gui/
        ...
        │   ├── transcode.py             # P1 groundwork: post-rip FLAC→MP3/WAV via ffmpeg (whipper path; not yet wired)
...
- **`flac_recompress.py`** — optional post-rip FLAC re-compression: `recompress_flac_files()` re-encodes each output FLAC at `flac -8 --verify` (lossless; `flac` preserves tags + embedded art) to a sibling temp, then `os.replace`s it in atomically, returning a `RecompressResult` (never raises; same couldn't-run vs per-file-failure split as `flac_verify`; a failed file is left untouched). Opt-in, off by default; only runs when `WhipperBackend.produces_max_compression_flac()` is False (whipper encodes at `-5`; cyanrip already maxes, so the GUI skips it). The GUI folds it into the post-rip tag/cover thread so it runs *after* those, on the final files.
- **`transcode.py`** (P1 groundwork — not yet wired to any rip path; v1 stays FLAC-only, Critical Rule #4) — post-rip FLAC→MP3/WAV transcode for the whipper path (whipper is FLAC-only): `transcode_files(paths, *, fmt, mp3_vbr_quality)` re-encodes each FLAC to a sibling MP3/WAV via ffmpeg (MP3 = libmp3lame VBR `-q:a` + tags/cover; WAV = `pcm_s16le -map 0:a`), atomic swap-in, **FLAC kept as the master**, returns a `TranscodeResult` (never raises; same split as the FLAC adapters). cyanrip emits MP3/WAV natively (`-o`), so this is whipper-only — the GUI picks via `WhipperBackend.native_output_formats()`. Design + decision gates: `docs/mp3-wav-support.md`.
