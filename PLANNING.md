...
- **`main_window.py`** — `MainWindow(QMainWindow, RipMixin, UpdateMixin, …)`, the **assembler**. Central widget is a vertical stack of: `DrivePicker`, `DiscInfoPanel`, `TrackTable`, `RipControls`, `RipProgress`. Owns construction, menus, signal wiring, the MusicBrainz slots, Settings, and `_DialogQueuedResolver`. Cohesive concern-groups are factored into mixins it inherits (KDD-19) so this file stays focused on wiring; the mixins' methods run with `self` being the window. Down from a 1707-line god-object.
...
- **Status:** done — helpers, `UpdateMixin`, `RipMixin` (MainWindow 1707 → ~1008). Remaining mixins (Provisioning / Drive / Dependency) tracked in TASKS; target a ~300-line assembler.
