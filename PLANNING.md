...
│   ├── ci.yml                           # NINE gating jobs: test (3.11-3.14 + coverage floor), lint,
│   │                                    #   typecheck, changelog, media-guard, pip-audit, gitleaks,
│   │                                    #   sbom, tests-touched. tests-touched is GATING since
│   │                                    #   2026-08-20 (this line called it advisory until 2026-09-13,
│   │                                    #   and omitted gitleaks and sbom entirely)
...
│   │   ├── upstream-modification-investigation.md # EAC-parity investigation (CTDB, the previous backend)
...
│   # (the shared ownership split, jointly owned), TEST-VERIFICATION-CHECKLIST.md
│   # (the Tier-2 self-audit Critical rule #7 names), cyanrip-known-issues.md
│   # (the evidence-backed hand-off record), and script-language.md (GENERATED by
│   # scripts/emit_script_language.py — do not hand-edit). Two of those four are
│   # named by this project's own Critical rules and by nothing in this file.
│
...
│   ├── check.py                         # ONE command for every CI gate, with each real exit code in a
│   │                                    #   final table — exists because `pytest | tail` reports tail's
│   │                                    #   status (got wrong 4x across 2 sessions). Never pipe pytest.
...
│   │                                    #   inbound provider contract. Keyed to THEIR releases, not ours,
│   │                                    #   so it is not part of the version-bump ordering.
│   ├── mutation_sweep.py                # the weekly mutation sweep (replaced mutmut, 2026-09-05)
│   ├── revert_probe.py                  # proves a test is not vacuous: applies a revert, runs the named
│   │                                    #   tests, restores, reports detected/VACUOUS/REFUSED/NO EVIDENCE
...
│   # file — including handshake.py, which runs the protocol Critical rule #12 is
│   # built on, and check.py, which CLAUDE.md tells every session to run. §2 (the
│   # module map) has a gate and is complete at 150/150; §1, in the same file, had
│   # none. A map is only ever wrong by omission.
...
│   └── fixtures/                        # legacy-format cd-info / drive-list / rip-log samples, cyanrip + EAC logs (+ README)
│
└── src/
    └── platterpus/
        ...
        ├── offset_config.py             # is a read offset configured? (the GUI's own config only)
        ...
        ├── ripper_exit.py               # what the ripper's exit status says about WHO ended the rip (a signal we did not send)
        ├── ripper_message_inventory.py  # GENERATED: every diagnostic cyanrip can print (provider contract P5)
        ├── ripper_log_settle.py         # wait (bounded) for the ripper to finish WRITING its log, before anything reads it
        ...
        │   ├── __init__.py
        │   ├── rip_backend.py           # RipBackend ABC (+ RipMetadata); the previous backend's impl removed 2026-06-30 (KDD-18)
        ...
        │   ├── ripper_wrapper_probe.py  # which link in the ripper chain fails to EXIT (host export / container entry / binary)
        ...
        │
        ├── parsers/                     # cyanrip/EAC/legacy-format stdout+log parsing (named-group regexes)
        │   ├── __init__.py
        │   ├── rip_log.py               # shared RipLog dataclasses + the legacy log format parser (cyanrip logs → cyanrip_log.py)
        │   ├── drive_list.py            # shared DriveDescriptor + the legacy drive-list format parser
        │   ├── cd_info.py               # shared DiscInfo + the legacy cd-info format parser
        ...
        │   ├── main_window_settings.py  # SettingsMixin — Settings' OK/Apply + saving one setting from its home
...
- **`offset_config.py`** — tells whether a read offset is configured (`is_offset_configured` — the GUI's `--offset` override **only**, because that is the one value that reaches cyanrip's `-s`) and how to describe the offset that will be applied (`describe_applied_offset`); backs the drive-setup wizard's first-run auto-offer. Pure, no I/O. **No other file is consulted:** until 2026-09-24 it also read a leftover per-drive offset from the config file of the ripper older versions used (`~/.config/whipper/whipper.conf`), as a reference line in the drive wizard and `--doctor` and as a drive-ledger seed. That read was removed; only the uninstaller still touches the file, to delete it (`deps/host_teardown.py`).
...
- **`parity.py`** — compares a rip's per-track Copy CRCs against an EAC baseline log (`output_reference/`, `docs/test-plan.md`); the bit-perfect-equivalence check. `decode_log_bytes()` sniffs the log encoding (EAC writes **UTF-16**; cyanrip and legacy-format logs UTF-8) so real EAC logs read correctly; format-agnostic across FLAC/WAV/MP3 (the Copy CRC is on the extracted PCM).
...
- **`ripper_log_settle.py`** — waits, bounded, until the ripper's log carries the ripper's own completion footer, so nothing reads a file that is still being written. **Free on the normal path** (a rip read to EOF already has its footer) and only does work after a cancel, where the process we signal is the host-exported Distrobox *wrapper* and the process that writes the log lives inside the container and outlives it — so the wrapper's exit is no evidence at all. The discriminator is therefore *did we read the ripper's output to EOF*, which is the one fact the rip worker holds and nothing downstream can recover. **Two outcomes, and no inference between them:** the footer appeared, or we still do not know. The tempting quiet-window heuristic — *it stopped growing, so the writer is done* — is measurably wrong here (the log went quiet at the cancel and stayed quiet 6.6 s, because nothing kills the reader until the GUI's force-stop rescue reaches it), and concluding "settled" from silence is exactly the defect this module exists to remove: on 2026-09-09 one early read published `verdict "failed"`, `health_status: null` and *"Conclusive status report : absent"* about a complete, correctly-signed archival log. Pure and interruptible — no Qt, an injected clock, and an abandon predicate the shutdown path sets so window close is never held by a footer that is not coming. `docs/testing.md` §5.bg.
- **`ripper_exit.py`** — what the ripper's exit status says about **who ended the rip**, as the sentence a user reads when the ripper died without printing why. cyanrip's own codes are 0–5 (the fork's provider contract derives its whole exit inventory from source), so 128+N through the Distrobox wrapper — or −N from `Popen` — is a death by signal N, and podman's own 125 is the container tool failing. **A signal comes from software**, so the one thing it can say with certainty is that the disc and the drive are not the cause; which software sent it is not knowable from a number, so the sentence names candidates and never asserts one. The worker passes in whether *it* stopped the ripper, because a stop we caused described as coming "from outside" would send the user looking for a culprit that is us. Written after the 2026-09-24 acceptance run, whose whole-disc rip was killed when the container was stopped underneath it and whose status line read *"Rip failed — no diagnosis was captured"* over an exit 137 that was the diagnosis (`docs/testing.md` §5.br).
...
- **`cover_art.py`** — backend-independent cover art (2026-06-13). `fetch_front_cover(release_id)` GETs the Cover Art Archive `/front` image (stdlib urllib, injectable fetcher, magic-byte sniff, every failure → `None`); `plan_actions(mode, ripper_fetches_art, release_id)` is the pure gate (no-op when the ripper fetches its own art or the disc was never identified); `apply_cover_art(...)` writes `cover.<ext>` and/or embeds via `MetaflacAdapter.embed_picture`. Closes the gap where cyanrip rips (and, before KDD-18, the previous backend's `--unknown` heals) had no art (they bypass the ripper's own MB/CAA lookup). Honors Critical Rule #5 — the GUI queries, never the ripper.
...
- **`rip_log.py`** — defines the shared `RipLog`/`TrackResult` dataclasses every rip log becomes, whichever ripper wrote it (the cyanrip parser fills them too), and parses the **legacy log format** — the per-rip `.log` of the ripper Platterpus used before cyanrip, still read so a library's older rips stay readable by the report, parity and audit tools: per-track CRCs, AccurateRip match status, AccurateRip confidence, read offset confirmation, total error count.
- **`drive_list.py`** — defines the shared `DriveDescriptor` (vendor, model, firmware, device path) and parses the legacy drive-list output format into a list of them — kept for old paths and test fixtures; cyanrip's drive listing is a backend-independent `/dev/sr*` + sysfs scan.
- **`cd_info.py`** — defines the shared `DiscInfo` (TOC, MusicBrainz disc ID, MB match status, AccurateRip availability) and parses the legacy cd-info output format into one — kept for old paths and test fixtures; cyanrip's disc info is parsed by `cyanrip_info.py`. `DiscInfo` is deliberately backend-neutral — it survived the swap to cyanrip unchanged (KDD-18).
...
- **`post_rip_record.py`** — `PostRipRecord`, a plain dataclass (no Qt) holding everything one finished album's rip report is built from, keyed by the rip generation that produced it. Every one of those facts used to be a `_last_*` attribute on `MainWindow`, whose lifetime is *the current rip* — so each moved out from under its readers the moment the next rip started. That one shape produced three measured defects in two days: a report describing a configuration its rip never ran under, a post-rip chain abandoned so that no derived file was written at all, and a chain that succeeded and had every result discarded 655 ms later. A record is opened at Start (freezing what the rip was *asked* to do), filled in at finish (what it *did*), and stays addressable after the next rip begins — which is what lets a late CTDB, FLAC-integrity, derived-verify, transcode or checksum result be written into the report of the album it actually describes. Session-level facts (`environment`, the dependency report, the log buffer) deliberately stay on the window: copying them here would be the same lifetime mistake mirrored.
- **`settings_dialog.py`** — `SettingsDialog(QDialog)`. **The rip's settings, and only those** (2026-09-24, `setting_homes.py`): the read offset is shown read-only and edited in the drive wizard, the update channels live in Setup & Updates, the test-script options in the console, and any field it does not edit is carried through from the config it opened with. **OK / Apply / Cancel / Restore Defaults**; Apply saves through the window and re-baselines, Restore Defaults resets only the controls homed here. One unified page: output dirs, track/disc templates, the metaflac path, output format, cover art, max-retries, secure re-rip, FLAC re-compress, CTDB/FLAC verify toggles, auto-launch-Picard, auto-eject. (The previous backend's force-overread / keep-going / CD-R widgets were removed with it — KDD-18.) Options the sole backend doesn't need are greyed out with a why+how-to tooltip (e.g. FLAC re-compress — cyanrip already maxes compression), and `to_config()` still reads disabled widgets so a value is never lost. Persists through `config.py`. (The old two-backend toggle and `_apply_backend_capabilities` were removed when cyanrip became the sole backend — KDD-18/21.)
...
- **`ripper_wrapper_probe.py`** — answers one question: *which link in the host→container→cyanrip chain fails to exit?* It times four invocations — the host export with stdin attached (the shape observed to hang), the same with stdin closed (the candidate one-character fix), `distrobox-enter -- true` (the wrapper carrying no cyanrip at all), and the in-container binary — then reports the **narrowest** conclusion they support, tri-state. It exists because two consecutive rig mornings produced zero rips without either program's rip path being involved: `cyanrip --version` printed its banner in full and then never returned. The cyanrip fork asked for three shell commands; `CLAUDE.md` forbids handing a procedure back, so the app runs them, reached by the `probe-ripper-wrapper` script verb and a `--doctor` row as a second thin caller of the same function. It is enumerated as a **ripper-capable spawn site** and delegates to `assert_metadata_lookup_disabled` rather than reasoning about `-N` itself.
...
exactly what let the unmaintained previous backend be removed cleanly on 2026-06-30 —
...
ABC (`adapters/rip_backend.py`) plus one concrete implementation — **cyanrip** (`adapters/cyanrip_backend.py`), the sole backend since the previous backend was removed 2026-06-30 (KDD-18). The whole point of the seam is that the GUI never has to know which ripper backs it — which is exactly what let the previous backend be swapped out for cyanrip without touching the GUI layer.
...
Built once the previous backend's cd-paranoia >587-offset bug failed real tracks on the BDR-209D.)*

- Implements the `RipBackend` ABC; it is now the **sole** backend (the previous backend removed 2026-06-30), hard-returned by `composition.py`'s `build_backend(cfg)` at startup (KDD-21 moved backend construction out of `app.py`; the `Config.ripper_backend` selector was removed with the previous backend).
...
- Flatpak: disqualified by the brief — the sandbox cannot reliably reach the host-exported ripper in `~/.local/bin/` (cyanrip today; the previous backend when this was written).
...
See `docs/eac-parity.md` for the full side-by-side. The comparison is anchored on a real upstream whipper test fixture (`tests/fixtures/rip_log_real_whipper_0_7.log`, renamed `rip_log_legacy_format.log` on 2026-09-24) and a representative EAC v1.6 log (`tests/fixtures/rip_log_eac_reference.log`, hand-authored to match Hydrogenaudio/CueTools documentation).
...
> | `-x` / `--cache-probe` | **the cyanrip fork only** | measures the drive's readback cache **and then rips the whole disc** — measured 2026-08-19; it is not a cheap probe. Single home: `docs/dependency-contracts.md` |
>
...
#### Moved from `CLAUDE.md` (2026-09-26): A NEW file is the last resort, and the commit must name the homes it rejected

*Verbatim from `CLAUDE.md` at `f5305a7` (Critical rules, rule #7, fourth obligation), moved here when that file was trimmed to its rules and pointers. Inside it, “here” and “this file” mean `CLAUDE.md`, and “above” / “below” mean the neighbouring entries of that section.*
...
test green in one run — achieved **at least twice, on at least TWO machines and
TWO distros**, counted over the full-green rows only. **1.0.0** additionally
requires independent field evidence: more than one **person**, machine and Linux
distribution — a *coverage* bar no amount of diligence on one rig can clear. The
full statement, the reasoning and the evidence ledger are `docs/testing.md` §5B;
`tests/test_no_stale_version_claims.py` §3 refuses a bump the ledger does not
support.

**Amended three times, and this KDD carried none of them until 2026-09-13** — which
is the decision log failing at the one thing it is for. All three were live in
`CLAUDE.md` and `docs/testing.md` while this entry described the original ruling:

* **2026-08-21 — the next minor is `0.7.100`, gated on a FULL hardware pass**
  (*"fresh start, rip, every test there is, all of them"*). The gate moved far
  earlier than 0.9.1 and this file did not mention `0.7.100` at all. The jump to
  `.100` is the maintainer's numbering, not a typo — do not "correct" it to 0.7.0.
* **2026-08-26 — "error free" is about what a failure MEANS, not how many there
  are.** Zero failures in the **archival** sections (accuracy, provenance, the
  records that make a rip trustworthy); UX failures recorded, triaged,
  non-blocking. **The classification is a property of the TEST, declared in
  advance** — never a judgement made about a failure after seeing it, because
  *"the five failures were each understood"* is the exact sentence 2026-08-19
  disproved. A UX failure stays non-blocking only while it is its **own** defect;
  sharing a root cause with an archival one makes it archival.
* **2026-09-13 — the diversity clause moved onto 0.9.1.** Previously every such
  clause sat on 1.0.0 and 0.9.1 was a pure count. The maintainer's words: *"me
  passing full tests, even if different, on the same version of linux and hardware
