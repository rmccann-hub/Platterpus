...
def test_our_artifacts_are_recognised_by_their_OWN_TEXT_not_their_name(
    tmp_path: Path, vls
) -> None:
    """**The third spelling, and the reason the name check is only a pre-filter.**

    One artifact, three names across two repositories:

    * `cyanrip_fork_police_classics_EACcompatible.log` — `output_reference/`
    * `… (EAC-compatible).log` — an evidence bundle
    * `after-cancel.eac.log` — where the cyanrip fork files our logs in *theirs*

    The name rule caught the first two and missed the third the moment the file
    crossed a repository boundary. That is exactly what `CLAUDE.md`'s
    cross-machine filename rule predicts, and the remedy it prescribes is the
    second half nobody does: *legislate the name **and** stop depending on it.*

    So the load-bearing check is what the document says about itself. Renaming it
    to anything at all cannot defeat that.
    """
    ours = tmp_path / "totally-innocent-name.log"
    ours.write_text(
    ...
    report = vls.sweep([ours, tmp_path / "real.log"])

    assert report.skipped_as_ours == [str(ours)]
    assert str(ours) not in report.logs_examined
    assert not report.unrecognised
    assert vls.main([str(tmp_path)]) == 0


def test_a_ripper_log_that_merely_QUOTES_our_banner_is_still_theirs(
    tmp_path: Path, vls
) -> None:
    """The converse, so the text check cannot swallow a real log.

    Our marker belongs in the first line and the footer. A cyanrip log that
    mentions the phrase far down — in a path, a tag, a quoted message — is still
    theirs and must still be graded, so only the head is scanned.
    """
    text = _CORPUS.read_text(encoding="utf-8", errors="replace")
    quoting = tmp_path / "theirs.log"
    quoting.write_text(
        text
        + "\nSomeone said: Exact Audio Copy-compatible log generated by Platterpus\n",
        encoding="utf-8",
    )
    assert not vls._is_ours_by_text(quoting.read_text())
    report = vls.sweep([quoting])
    assert report.skipped_as_ours == []
...
def test_skipped_files_are_listed_rather_than_vanishing(tmp_path: Path, vls) -> None:
    ...
    `CLAUDE.md`: a silent truncation reads as completeness. Anything set aside is
    counted and named, the same obligation the unreadable-log case carries.
    """
    ours = tmp_path / "x.log"
    ours.write_text(
        "Exact Audio Copy-compatible log generated by Platterpus\n" + "a\n" * 60
    ...
    assert report.skipped_as_ours == [str(ours)]
    assert report.logs_examined == []
    # ...and with nothing else to look at, that is UNPROBED rather than a pass.
    assert vls.main([str(tmp_path)]) == 2
