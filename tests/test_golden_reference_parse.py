...
#: The round-7 pair (lap 12 and lap 14). **Not "every committed reference"**, which
#: this comment claimed until 2026-09-23 while twenty-odd more sat beside it: the
#: checks that iterate over it assert round-7 facts — that pair's consumer string,
#: its delivering laps — so they could never have held for later references. A
#: later reference gets its own assertions, derived from its own text, as round 24's
#: does below.
...
# --- Round 24: the `+platterpus.14` reference at the pin under review ----------
#
# Round 24's ONE close condition is our verdict on `3e01bb3`, resting on "your
# parser at 0.6.53 or later having read a log `3e01bb3` wrote" — and they declared
# this reference sufficient. So this is the measurement the verdict rests on,
# committed rather than reported once. Generated by `2e6d97d`, whose `src/` and
# `meson.build` equal `3e01bb3`'s (`git diff --stat 2e6d97d 3e01bb3 -- src/
# meson.build` is empty); committed in their tree at `02bc6a7`.

GOLDEN_R24 = _ARTIFACTS / "round-24-lap-01-golden-reference-g2e6d97d.log"
GOLDEN_R24_SHA256 = "14a778166cab78a734ac00bcc60e258db398d12948c2886fad7ac78381d51573"


def test_the_round_24_reference_is_the_file_they_declared() -> None:
    import hashlib

    assert GOLDEN_R24.is_file(), f"missing {GOLDEN_R24}"
    assert hashlib.sha256(GOLDEN_R24.read_bytes()).hexdigest() == GOLDEN_R24_SHA256


def test_the_round_24_reference_parses_COMPLETELY_under_the_new_wording(
    caplog: pytest.LogCaptureFixture,
) -> None:
    """Every expectation is read out of the artifact's own text, so it cannot
    drift from it — and "completely" is measured, not assumed: the parser reports
    unrecognised top-level lines at DEBUG, and there must be none."""
    import logging

    text = _text(GOLDEN_R24)
    new_wording = re.findall(r"^Track \d+ read successfully!$", text, re.M)
    assert len(new_wording) == 3, new_wording
    assert "ripped and encoded" not in text, "the reference carries the OLD wording"
    assert re.search(r"^Encoder errors: none; 3 tracks encoded$", text, re.M)
    with caplog.at_level(logging.DEBUG, logger="platterpus.parsers.cyanrip_log"):
        parsed = parse_cyanrip_log(text)
    assert len(parsed.tracks) == len(new_wording)
...
@pytest.mark.parametrize(
    ("find", "replace", "expect_health", "expect_status_2"),
    [
        (
            "Encoder errors: none; 3 tracks encoded",
            "Encoder errors: 2 tracks failed (2, 3); 1 track encoded",
            "2 encoder errors",
            "ripped successfully",
        ),
        (
    ...
    ids=["encoder-failed-arm", "encoder-not-applicable-arm", "read-with-errors"],
)
def test_the_round_24_footer_arms_and_error_wording_on_the_real_log(
    find: str, replace: str, expect_health: str, expect_status_2: str
) -> None:
    """The arms their golden reference does not exercise, applied to it.

    Their round 24 lap 1 §F: *"`Track %i read with errors.` is asserted by no test
    of ours"*. It is asserted here, on their own log's shape. And the failed
    encoder arm must make the disc NOT clean — `No errors occurred` is the string
    downstream treats as clean — while `not applicable` must not read as a failure.
    """
    text = _text(GOLDEN_R24)
