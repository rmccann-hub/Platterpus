...
# --- Legacy config-dir migration (Platterpus rename) ----------------------


def test_migrates_legacy_whipper_gui_config(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A pre-rename ~/.config/whipper-gui is adopted when the new dir is absent."""
    new_dir = tmp_path / "platterpus"
    legacy = tmp_path / "whipper-gui"
    legacy.mkdir()
    (legacy / "config.toml").write_text('output_dir = "/music/legacy"\n')
    monkeypatch.setattr(config_module, "CONFIG_DIR", new_dir)
    monkeypatch.setattr(config_module, "CONFIG_PATH", new_dir / "config.toml")
    monkeypatch.setattr(config_module, "LEGACY_CONFIG_DIR", legacy)

    assert config_module.migrate_legacy_config_dir() is True
    cfg = config_module.load()
    assert cfg.output_dir == "/music/legacy"  # the user's old setting survived
    assert legacy.exists()  # copied, not moved


def test_no_migration_when_new_dir_exists(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Never clobber an existing Platterpus config with the legacy one."""
    new_dir = tmp_path / "platterpus"
    new_dir.mkdir()
    (new_dir / "config.toml").write_text('output_dir = "/music/new"\n')
    legacy = tmp_path / "whipper-gui"
    legacy.mkdir()
    (legacy / "config.toml").write_text('output_dir = "/music/legacy"\n')
    monkeypatch.setattr(config_module, "CONFIG_DIR", new_dir)
    ...
    assert config_module.migrate_legacy_config_dir() is False
    assert config_module.load().output_dir == "/music/new"


def test_no_migration_when_no_legacy_dir(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(config_module, "CONFIG_DIR", tmp_path / "platterpus")
    monkeypatch.setattr(
        config_module, "CONFIG_PATH", tmp_path / "platterpus" / "c.toml"
    )
    monkeypatch.setattr(config_module, "LEGACY_CONFIG_DIR", tmp_path / "whipper-gui")
    assert config_module.migrate_legacy_config_dir() is False
